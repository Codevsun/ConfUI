...
- [ ] M3: Read-only TUI — open file from CLI arg (clap), render tree
...
- [ ] M4: Editing — Enter to edit inline, type-aware inputs (bool toggle
      with Space, string/number inputs with validation), modified
      indicator, Ctrl+S save with .bak backup + atomic write.
- [ ] M5: Undo/redo — command pattern in history/, Ctrl+Z / Ctrl+Y.
